...
class SettingsManager {
    ...
    // Novas funções para gerenciar filas na tela
    async loadDisplayQueues() {
        try {
            Utils.showLoading(true, 'Carregando filas da tela...');
            const branchId = localStorage.getItem('branchId') || sessionStorage.getItem('branchId');
            const response = await axios.get(`${API_BASE}/api/totem/branches/${branchId}/display_queues`, {
                params: { refresh: true }
            });
            ...
            queues.forEach(queue => {
                const queueCard = document.createElement('div');
                queueCard.className = 'flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-100';
                queueCard.innerHTML = `
                    <div>
                        <p class="font-medium">${queue.service_name}</p>
                        <p class="text-xs text-gray-500">Prefixo: ${queue.prefix}</p>
                        <p class="text-xs text-gray-500">Ordem: ${queue.display_order}</p>
                    </div>
                    <button class="remove-queue-btn text-red-500 hover:text-red-700 p-1 rounded-full hover:bg-red-50" data-id="${queue.queue_id}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            document.querySelectorAll('.remove-queue-btn').forEach(btn => {
                btn.addEventListener('click', () => this.removeQueueFromDisplay(btn.dataset.id));
            });
        } catch (error) {
            console.error('Erro ao carregar filas da tela:', error);
            Utils.showToast('Erro ao carregar filas da tela', 'error');
        } finally {
            Utils.showLoading(false);
        }
    }

    async addQueueToDisplay(queueId) {
        try {
            Utils.showLoading(true, 'Adicionando fila à tela...');
            const branchId = localStorage.getItem('branchId') || sessionStorage.getItem('branchId');
            const response = await axios.post(`${API_BASE}/api/branch_admin/branches/${branchId}/display/queues`, { queue_id: queueId });
            Utils.showToast(response.data.message || 'Fila adicionada à tela com sucesso', 'success');
            document.getElementById('add-display-queue-modal').classList.add('hidden');
            this.loadDisplayQueues();
        } catch (error) {
    ...
    async removeQueueFromDisplay(queueId) {
        try {
            Utils.showLoading(true, 'Removendo fila da tela...');
            const branchId = localStorage.getItem('branchId') || sessionStorage.getItem('branchId');
            const response = await axios.delete(`${API_BASE}/api/branch_admin/branches/${branchId}/display/queues/${queueId}`);
            Utils.showToast(response.data.message || 'Fila removida da tela com sucesso', 'success');
            this.loadDisplayQueues();
        } catch (error) {
    ...
    async loadAvailableQueuesForDisplay() {
        try {
            const branchId = localStorage.getItem('branchId') || sessionStorage.getItem('branchId');
            const response = await axios.get(`${API_BASE}/api/branch_admin/branches/${branchId}/queues`);
            const queues = response.data;
            const select = document.getElementById('display-queue-id');
            select.innerHTML = '<option value="">Selecione uma fila</option>';
            queues.forEach(queue => {
                const option = document.createElement('option');
                option.value = queue.id;
            ...
            console.error('Erro ao carregar filas disponíveis para a tela:', error);
            Utils.showToast('Erro ao carregar filas disponíveis', 'error');
        }
    }

    showAddDisplayQueueModal() {
        document.getElementById('add-display-queue-modal').classList.remove('hidden');
        this.loadAvailableQueuesForDisplay();
    }
}
